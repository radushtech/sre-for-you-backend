...
app.post('/api/generate-roadmap', async (req, res) => {
  try {
    ...
    if (aiData.error) {
      return res.status(502).json({ error: aiData.error.message || 'Anthropic API error' });
    }
    // Same contract fix as generate-roadmap-public: frontend expects
    // { text: "..." }, not the raw Anthropic message object.
...
app.post('/api/generate-roadmap-public', publicRoadmapLimiter, async (req, res) => {
  try {
    ...
    // The frontend (assessment3.html) expects { text: "..." }, not the raw
    // Anthropic message object — it reads data.text directly and JSON.parses
    // it. Extract and flatten the text content here so the contract matches.
    const text = (aiData.content || []).map(c => c.text || '').join('').trim();
    res.json({ text });
  } catch (err) {
    console.error(err);
